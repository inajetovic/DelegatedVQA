...
class Client:
    ...
    def delegate_test_run(self, outnodes,angles_noise,backend: Backend, run: TrappifiedCanvas, trapinfo=False, **kwargs) -> list[int]:
        ...
        for node in self.measurement_db:
            ...
            if node in outnodes:
                self.measurement_db[node] = graphix.command.M(node=node,angle=angles_noise)
            else:
                self.measurement_db[node] = graphix.command.M(node=node,angle=0)
